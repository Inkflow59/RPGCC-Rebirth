...
public class ObjectSelection
{
	...
	public boolean filterQualifierObjects(Object rdPtr, int OiList, boolean negate, Filter filter)
	{
		...
		while( i < length)
		{
			int noil = CurrentQualToOi.qoiList[i+1];
			if (noil < 0)
				break;

			hasSelected = (((hasSelected ? 1 : 0) |
					(filterNonQualifierObjects(rdPtr, noil, negate, filter) ? 1 : 0))) != 0;
	...
	public int getNumberOfSelected(int OiList)
	{
		if((OiList & 0x8000) != 0)
		{
			...
			while( i < length)
			{
				...
				if (noil < 0)
					break;
				CObjInfo CurrentOi = GetOILFromOI(noil);
				if(CurrentOi == null)
					return 0;
	...
	public boolean objectIsOfType(CObject obj, int OiList)
	{
		if((OiList & 0x8000) != 0)
		{
			...
			while( i < CurrentQualToOi.qoiList.length)
			{
				...
				if (noil < 0)
					break;
				CObjInfo CurrentOi = run.rhOiList[noil]; //GetOILFromOI(Oi);
				if (CurrentOi == null)
					return false;
