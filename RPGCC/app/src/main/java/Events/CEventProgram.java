...
import Conditions.IEvaExpObject;
import Conditions.IEvaExpObjectDouble;
...
public class CEventProgram implements IEnum
{
    public static final short EVENTS_EXTBASE = 80;

	public static final short CNDIDX_EXTONLOOP_INTERNAL_FIRST = 52;								// 1-based index of first available common condition
	public static final short CNDIDX_EXTONLOOP_INTERNAL_LAST = 	CEventProgram.EVENTS_EXTBASE;	// 1-based index of last common condition
	public static final short CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER = (CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_LAST + 1 - CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_FIRST);
	public static final short NUMBER_OF_ACTIVEOBJECT_CONDITIONS = 4;
	public static final short CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS = 199 + CEventProgram.EVENTS_EXTBASE;
	public static final short CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER_ONLYACTIVEOBJECTS = (CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS + 1 - CEventProgram.NUMBER_OF_ACTIVEOBJECT_CONDITIONS - CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_FIRST);
    ...
    public short eventPointersCnd[] = null;
    ...
    public ArrayList<Map <CObjInfo, ArrayList<Integer>>> childEventSelectionStack = new ArrayList<>();
    public int defaultEvgFlagsMask = CEventGroup.EVGFLAGS_DEFAULTMASK;          // Default flags for eventgroups
	public Map <String, Integer> onEachLoopConditionIndexes = null;
	public Map <String, Integer> onEachLoopConditionIndexesActives = null;
    public boolean allOnEachLoopsAreOptimized = false;
    public CQualToOiList qualToOiListFull[] = null;
    ...
    public CObject evt_NextObject()
    {
        ...
        do
        {
            ...
            short noil = rh2EventQualPos.qoiList[rh2EventQualPosNum + 1];
            if (noil < 0)
            {
                return null;
            }
            oilPtr = rhPtr.rhOiList[noil];
    ...
    public void evt_AddCurrentQualifier(short qual)
    {
        ...
        CObjInfo oilPtr;
        int pQoi_qoiList_length = pQoi.qoiList.length;
        while (nq < pQoi_qoiList_length)
        {
            short noil = pQoi.qoiList[nq + 1];
            if (noil < 0)
                break;
            oilPtr = rhPtr.rhOiList[noil];
            if (oilPtr.oilEventCount != rh2EventCount)
...
//            rhPtr.rhOiList[rh2EventPosOiList].oilListSelected=rh2EventPos.hoNextSelected;
    ...
    public void evt_DeselectObject (short oil)
    {
        ...
        if (oil >= 0) {
            ...
            if (oil == -1)
                return;
            // Qualifier
            CQualToOiList pqoi = this.qualToOiList[oil & 0x7FFF];
            int qoi;
            int pqoi_qoiList_length = pqoi.qoiList.length;
            for (qoi = 0; qoi < pqoi_qoiList_length; qoi += 2) {
                short oilq = pqoi.qoiList[qoi + 1];
                if (oilq < 0)
                    break;
                CObjInfo poil = rhPtr.rhOiList[oilq];
                poil.oilEventCount = rh2EventCount;
                poil.oilListSelected = -1;
                poil.oilNumOfSelected = 0;
            }
    ...
    public void evt_SelectObjects (short commonOil, ArrayList<CObject> hOList)
    {
        ...
        short oilLast = -1;
        CObject pHoLast = null;
        int index;
        for (index = 0; index < hOList.size(); index++) {
            CObject pHo = hOList.get(index);
    ...
    public void evt_SaveSelectedObjects(short oiOilList[], Map <CObjInfo, ArrayList<Integer>> selectedObjects)
    {
        for (int n = 0; n < oiOilList.length; n += 2)
        {
            ...
            {
                // Already in the list?
                ArrayList<Integer> selectedInstances = selectedObjects.get(poil);
                if (selectedInstances != null)
                    selectedInstances.clear();
                else {
                    selectedInstances = new ArrayList<Integer>();
                    selectedObjects.put(poil, selectedInstances);
                }
    ...
    public void evt_RestoreSelectedObjects(short oiOilList[], Map <CObjInfo, ArrayList<Integer>> selectedObjects)
    {
        for (int n = 0; n < oiOilList.length; n += 2)
        {
            short oil = oiOilList[n + 1];
            ...
            if (selectedInstances != null)
            {
                int selectedInstancesSize = selectedInstances.size();
                poil.oilEventCount = rh2EventCount;        // TODO: SetObjectEventCount(pRh, poil);
                poil.oilListSelected = -1;
                poil.oilNumOfSelected = 0;
                if (selectedInstancesSize > 0)
                {
                    CObject pHoPrev = rhPtr.rhObjectList[selectedInstances.get(0)];
                    if (pHoPrev != null)
                    {
                        poil.oilListSelected = selectedInstances.get(0).shortValue();
                        poil.oilNumOfSelected++;
                        for (int j = 1; j < selectedInstancesSize; j++)
                        {
                            short num = selectedInstances.get(j).shortValue();
    ...
    public void executeChildEvents(PARAM_CHILDEVENT eventParam)
    {
        ...
        Map <CObjInfo, ArrayList<Integer>> selectedObjects = new HashMap<>();
        if (newIdx > 0) {
            // Copy previous selection
            Map <CObjInfo, ArrayList<Integer>> prevSelectedObjects = childEventSelectionStack.get(newIdx - 1);
			for (CObjInfo poil : prevSelectedObjects.keySet()) {
				ArrayList<Integer> selectedInstances = new ArrayList<Integer>(prevSelectedObjects.get(poil));
                selectedObjects.put(poil, selectedInstances);
			}	            
    ...
    public boolean handle_Event(CObject pHo, int code)
    {
        ...
        {
            this.computeEventList(num, pHo);     // Evalue les evenements
            return true;
        }
        return false;
    }
    ...
    public int updateZoneCoordinates(CParam params[], int n)
    {
        PARAM_ZONE p = (PARAM_ZONE)params[n];
        if (p.code == 72)       // PARAM_ZONE_EXPRESSION
        {
            p.x1 = rhPtr.get_EventExpressionInt((CParamExpression)params[n+1]);
            p.y1 = rhPtr.get_EventExpressionInt((CParamExpression)params[n+2]);
            p.x2 = rhPtr.get_EventExpressionInt((CParamExpression)params[n+3]);
            p.y2 = rhPtr.get_EventExpressionInt((CParamExpression)params[n+4]);
            return 5;
        }
        else
            return 1;
    ...
    public CObject count_ObjectsFromOiList(short oil, int stop)
    {
        ...
        for (qoi = 0; qoi < pqoi_qoiList_length; qoi += 2)
        {
            ...
            if (noil < 0)
                break;
            CObject pHo = countThem(noil, stop);
            if (pHo != null)
            {
                return pHo;
            }
    ...
    public void handle_PushedEvents()
    {
        ...
        {
            int index;
            int pushedMax = rh2PushedEvents.size();
            for (index = 0; index < pushedMax; index++)
            {
    ...
    public void load(CRunApp app)
    {
        ...
        while (true)
        {
            ...
            if (code[0] == 'E' && code[1] == 'R' && code[2] == '>' && code[3] == '>')
            {
				...
				if ( nConditions[COI.NUMBEROF_SYSTEMTYPES + COI.OBJ_SPR] < CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS+1)
					nConditions[COI.NUMBEROF_SYSTEMTYPES + COI.OBJ_SPR] = CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_LAST_ONLYACTIVEOBJECTS+1;
    ...
    public void prepareQualifierList(CObjInfo[] oiList)
	{
        qualToOiList = null;
        ...
        if (nQualifiers > 0)
        {
			...
			int q;
	        int oil;
			short qualifierNumber;
            short count[] = new short[nQualifiers];
            for (CObjInfo oilPtr : oiList) {
				oilPtr.oilEventCount = 0;
                oilPtr.oilRealQualifiers = null;
            }
            for (q = 0; q < nQualifiers; q++)
            {
                qualifierNumber = (short) ((qualifiers[q].qOi) & 0x7FFF);
                count[q] = 0;
                for (CObjInfo oilPtr : oiList)
                {
                    if (oilPtr.oilType == qualifiers[q].qType)
                    {
            ...
            qualToOiListFull = new CQualToOiList[nQualifiers];
            for (q = 0; q < nQualifiers; q++)
            {
                qualToOiList[q] = new CQualToOiList();
                qualToOiListFull[q] = new CQualToOiList();

				int cnt = count[q];
                if (cnt != 0)
                {
                    qualToOiList[q].qoiList = new short[cnt * 2];
                    qualToOiListFull[q].qoiList = new short[cnt * 2];
                }
                ...
                for (CObjInfo oilPtr : oiList)
                {
                    if (oilPtr.oilType == qualifiers[q].qType)
                    {
                        for (n = 0; n < 8 && oilPtr.oilQualifiers[n] != -1; n++)
                        {
                            if (qualifierNumber == oilPtr.oilQualifiers[n])
                            {
                                qualToOiListFull[q].qoiList[i * 2] = oilPtr.oilOi;
								...
								if (oilPtr.oilRealQualifiers == null)
								{
									...
									{
                                        oilPtr.oilRealQualifiers = new short[oilPtr.oilEventCount];
										oilPtr.oilEventCount = 0;
									}
								}
								if (oilPtr.oilRealQualifiers != null)
									oilPtr.oilRealQualifiers[oilPtr.oilEventCount++] = (short)q;
                                i++;
                                break;
                            }
                        }
    ...
    public void assemblePrograms(CRun run)
    {
                ...
                if (type >= COI.OBJ_SPR)
                {
                    switch (getEventCode(code))
                    {
                        ...
                        case (-14 << 16):     // CNDL_EXTCOLLISION:
                            ...
                            if ((rhPtr.rhApp.hdr2Options2 & CRunApp.AH2OPT2_WINDOWSLIKEBOXCOLLISIONS) != 0) {
                                replaceBoxCollisionModes(evtPtr.evtOiList, evtPtr.evtOi, ((PARAM_OBJECT)evpPtr).oiList, ((PARAM_OBJECT)evpPtr).oi);
                            }
		...
		Boolean bOptimizeForEach = false;
		if (onEachLoopConditionIndexes == null && onEachLoopConditionIndexesActives == null)	// do it only if not already done
		{
			onEachLoopConditionIndexes = new HashMap <> (32);
			onEachLoopConditionIndexesActives = new HashMap <> (32);
			bOptimizeForEach = true;
			allOnEachLoopsAreOptimized = true;

			// Find all On Each loop names, and test if all On Each conditions use constant names and there is no For Each of Two action
			for (evg = 0; evg < events.length && bOptimizeForEach; evg++)
			{
				evgPtr = events[evg];
				if(evgPtr == null)
            		continue;
				...
				for (evt = 0; evt < evgPtr.evgNCond; evt++)
				{
					...
					type = EVTTYPE(evtPtr.evtCode);
					if (type >= COI.OBJ_SPR && getEventCode(evtPtr.evtCode) == (-41 << 16))	// CNDL_EXTONLOOP)
					{
						// Name = simple string?
						CParamExpression pExp = (CParamExpression) evtPtr.evtParams[0];
						if (pExp.tokens.length == 2 && pExp.tokens[0].code == ((3 << 16) | 65535) && pExp.tokens[1].code==0)
						{
							String loopname = ((EXP_STRING)pExp.tokens[0]).string.toLowerCase ();
							Map <String, Integer> conditionUseCountMap = (type == COI.OBJ_SPR) ? onEachLoopConditionIndexesActives : onEachLoopConditionIndexes;
							Integer cndUseCount = conditionUseCountMap.get(loopname);
							if (cndUseCount == null)
								cndUseCount = Integer.valueOf(1);
							else
								cndUseCount = Integer.valueOf(cndUseCount.intValue()+1);
							conditionUseCountMap.put(loopname, cndUseCount);
						}
						else
						{
				...
				for (n = 0; n < evgPtr.evgNAct && bOptimizeForEach; n++)
				{
					...
					type = EVTTYPE(evtPtr.evtCode);
					if (type >= COI.OBJ_SPR && getEventCode(evtPtr.evtCode) == (77 << 16))		// ACT_EXTFOREACH2)
					{
						// Can't use For Each optimization
						bOptimizeForEach = false;
						break;
			...
			if (!bOptimizeForEach)
			{
				onEachLoopConditionIndexes = null;
				onEachLoopConditionIndexesActives = null;
				allOnEachLoopsAreOptimized = false;
			}
			else
			{
				// If the number of loop names exceeds the maximum available condition numbers, remove the ones with the lower use count
				while (onEachLoopConditionIndexes.size() > CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER)
				{
					int minUseCount = 1000000000;
					String minLoopName = null;
					for (String loopname : onEachLoopConditionIndexes.keySet()) {
						Integer useCount = onEachLoopConditionIndexes.get(loopname);
						if (useCount != null && useCount.intValue() < minUseCount)
						{
							minLoopName = loopname;
							minUseCount = useCount.intValue();
						}
					}	            
					if (minLoopName != null)
					{
						onEachLoopConditionIndexes.remove(minLoopName);
						allOnEachLoopsAreOptimized = false;
					}
				}

				// Transform counts to condition numbers in onEachLoopConditionIndexes map
				int cndIndex = CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_FIRST;
				for (String loopname : onEachLoopConditionIndexes.keySet()) {
					onEachLoopConditionIndexes.put(loopname, Integer.valueOf(cndIndex++));
				}	            

				// Same thing for active object loops
				// If the number of loop names exceeds the maximum available condition numbers, remove the ones with the lower use count
				while (onEachLoopConditionIndexesActives.size() > CEventProgram.CNDIDX_EXTONLOOP_INTERNAL_MAXNUMBER_ONLYACTIVEOBJECTS)
				{
					int minUseCount = 1000000000;
					String minLoopName = null;
					for (String loopname : onEachLoopConditionIndexesActives.keySet()) {
						Integer useCount = onEachLoopConditionIndexesActives.get(loopname);
						if (useCount != null && useCount.intValue() < minUseCount)
						{
							minLoopName = loopname;
							minUseCount = useCount.intValue();
					...
					if (minLoopName != null)
					{
						onEachLoopConditionIndexesActives.remove(minLoopName);
						allOnEachLoopsAreOptimized = false;
					}
				}
				...
				for (String loopname : onEachLoopConditionIndexesActives.keySet()) {
					onEachLoopConditionIndexesActives.put(loopname, Integer.valueOf(cndIndex++));
				...
				if (cndIndex > CEventProgram.EVENTS_EXTBASE + 1 + CEventProgram.NUMBER_OF_ACTIVEOBJECT_CONDITIONS)
					nConditions[COI.NUMBEROF_SYSTEMTYPES + COI.OBJ_SPR] += cndIndex - (CEventProgram.EVENTS_EXTBASE + 1 + CEventProgram.NUMBER_OF_ACTIVEOBJECT_CONDITIONS);
			}
		}
        ...
        for (evg = 0; evg < nEvents; evg++)
        {
            ...
            for (evt = 0; evt < evgPtr.evgNCond; evt++)
            {
				...
				// For each loop optimization
				// On Each condition?
				if (bOptimizeForEach && type >= COI.OBJ_SPR && getEventCode(code) == (-41 << 16))	// CNDL_EXTONLOOP)
				{
					// Name = simple string?
					CParamExpression pExp = (CParamExpression) evtPtr.evtParams[0];
					if (pExp.tokens.length == 2 && pExp.tokens[0].code == ((3 << 16) | 65535) && pExp.tokens[1].code==0)
					{
						...
						String loopname = ((EXP_STRING)pExp.tokens[0]).string.toLowerCase ();
						Map <String, Integer> conditionIndexMap = (type == COI.OBJ_SPR) ? onEachLoopConditionIndexesActives : onEachLoopConditionIndexes;
						Integer numInt = conditionIndexMap.get(loopname);
						if (numInt != null)
						{
							num = numInt.intValue();
        ...
        eventPointersCnd = new short[sEventPointers];
                ...
                if (bOrBefore && evtPtr.evtCode != ((-42 << 16) | 0xFFFF))
                {
                    ...
                    if ((evtPtr.evtFlags & CEvent.EVFLAGS_ALWAYS) != 0)
                    {
                        ...
                        else
                        {
                            ...
                            eventPointersCnd[evtAlwaysPos] = (short)evt;
                            evtAlwaysPos++;
                        }
                    }
                    ...
                    if (type < 0)
                    {
                        ...
                        eventPointersCnd[lposBuffer[lposPtr]] = (short)evt;
                        lposBuffer[lposPtr]++;
                        ...
                        if (evtPtr.evtCode == ((-16 << 16) | 0xFFFF)) // CCnd.CND_ONLOOP
                        {
                            ...
                            if ( pExp.tokens[0].code == ((3 << 16) | 0xFFFF) && pExp.tokens[1].code == 0 )
                            {
                                ...
                                rhPtr.addFastLoop(pName);
                                int posStartLoop_size = posStartLoop.size();
                                for (nStartLoop = 0; nStartLoop < posStartLoop_size; nStartLoop++)
                                {
                                    CPosStartLoop pPosStartLoop = posStartLoop.get(nStartLoop);
                                    if (pPosStartLoop.m_name.equalsIgnoreCase(pName))
                                    {
                                        ...
                                        if (posOnLoop == null)
                                        {
                                            ...
                                            for (nOnLoop = 0; nOnLoop < rh4PosOnLoop_size; nOnLoop++)
                                            {
                                                posOnLoop = rh4PosOnLoop.get(nOnLoop);
                                                if (pName.equalsIgnoreCase(posOnLoop.m_name))
                                                    break;
                                            }
                                            if (nOnLoop == rh4PosOnLoop_size)
                                            {
                                        ...
                                        pPosStartLoop.m_expPtr.comparaison = (short)(nOnLoop + 1);
                                    }
                                }
                                if (posOnLoop == null)
                                {
                ...
                if (evtPtr.evtCode == ((-42 << 16) | 0xFFFF))  // CNDL_ENDCHILDEVENT
                {
                    ...
                    if (nestedEvgOffsetStack.size() > 0)
                    {
                        ...
                        for (int v = 0; v < curEvgList.size(); v++) {
                            ...
                            eventPointersCnd[evtAlwaysChildrenPos] = (short)curEvtList.get(v).intValue();
    ...
    public void AddColList(short oiList, short oiNum, short oiList2, short oiNum2) {
        ...
        int rhGameFlags = rhPtr.rhGameFlags;

        // Must not do it in fade in mode
        if ((rhGameFlags & CRun.GAMEFLAGS_FIRSTLOOPFADEIN) != 0)
            return;

        // First object = qualifier?
        if (oiNum < 0) {
            if (qualToOiListFull != null) {
                CQualToOiList qoil = qualToOiListFull[oiList & 0x7FFF];
                for (int index = 0; index < qoil.qoiList.length; index += 2)
        ...
        int idx = 0;
        short[] oilColList = oilPtr.oilColList;
        if (oilColList == null) {
            oilColList = new short[2];
            oilPtr.oilColList = oilColList;
        } else {
            // Exit if object already in list
            int n;
            idx = oilColList.length;
            for (n = 0; n < idx; n += 2) {
                if (oiNum2 == oilColList[n]) {
                    return;
                }
            }

            short[] newList = Arrays.copyOf(oilColList, idx + 2);
            oilPtr.oilColList = newList;
            oilColList = newList;
        }

        oilColList[idx] = oiNum2;
        oilColList[idx + 1] = oiList2;
        //System.out.println("time addcollist is:"+(System.nanoTime()-starttime));
    }

    public void replaceBoxCollisionModes(short oiList, short oiNum, short oiList2, short oiNum2)
    {
        // Must not do it in fade in mode
        if ( (rhPtr.rhGameFlags & CRun.GAMEFLAGS_FIRSTLOOPFADEIN) != 0 )
            return;
        ...
        if ( oiNum < 0 )
        {
            if ( qualToOiListFull != null )
            {
                ...
                for (index=0; index<qoil.qoiList.length; index+=2)
                    replaceBoxCollisionModes(qoil.qoiList[index+1], qoil.qoiList[index], oiList2, oiNum2);
            }
            return;
        }
        ...
        if ( oiNum2 < 0 )
        {
            if ( qualToOiListFull != null )
            {
                ...
                int index;
                for (index=0; index<qoil.qoiList.length; index+=2)
                    replaceBoxCollisionModes(oiList, oiNum, qoil.qoiList[index+1], qoil.qoiList[index]);
            }
            return;
        }
        ...
        if (oilPtr.oilType == COI.OBJ_SPR && oilPtr2.oilType == COI.OBJ_SPR) {
            ...
            if ((oilPtr.oilOCFlags2 & CObjectCommon.OCFLAGS2_COLBOX) != (oilPtr2.oilOCFlags2 & CObjectCommon.OCFLAGS2_COLBOX)) {
                ...
                if ((ocPtr.ocOEFlags & CObjectCommon.OEFLAG_MOVEMENTS) != 0 && ocPtr.ocMovements != null && ocPtr.ocMovements.nMovements > 0) {
                    CMoveDef mvPtr = (CMoveDef)(ocPtr.ocMovements.moveList[0]);
                    if (mvPtr.mvType == CMoveDef.MVTYPE_EXT) {
                        CMoveDefExtension mvPtrExt = (CMoveDefExtension)(mvPtr);
                        isPhysics = mvPtrExt.isPhysics;
                    }
                }
                if (!isPhysics) {
                    // No physics movement
                    ...
                    while (i >= 0) {
                        CActive pHo = (CActive)rhPtr.rhObjectList[i];
                        if (pHo != null && pHo.roc != null && pHo.roc.rcSprite != null)
                            pHo.roc.rcSprite.sprFlags &= ~CSprite.SF_COLBOX;
                        i = pHo.hoNumNext;
                    }

                    // Enumerate images and set their opaque_for_collisions flag
                    if (rhPtr.rhApp.OIList.oiLoaded != null && rhPtr.rhApp.OIList.oiLoaded[oi] != 0)
    ...
    {
        CImage image = rhPtr.rhApp.imageBank.getImageFromHandle(num);
		if (image != null)
			image.setOpaqueMask(true);
        return -1;
    }
    ...
    short qual_GetNextOiList()
    {
        ...
        */
        if (qualOilPtr == -1 || qualOilPos >= qualToOiListFull[qualOilPtr].qoiList.length)
        {
            return -1;
        }
    ...
    short qual_GetNextOiList2()
    {
        ...
        */
        if (qualOilPtr2 == -1 || qualOilPos2 >= qualToOiListFull[qualOilPtr2].qoiList.length)
        {
            return -1;
        }
    ...
    int make_ColList1(CEventGroup evgPtr, int colList, short oi1) {
        ...
        int evgNCond = evgPtr.evgNCond;
        CEvent[] evgEvents = evgPtr.evgEvents;

        for (int evt = 0; evt < evgNCond; evt++) {
            CEvent evtPtr = evgEvents[evt];
            if (EVTTYPE(evtPtr.evtCode) >= 2) {
                ...
                switch (code) {
                    case (-14 << 16): // CNDL_EXTCOLLISION:
                        evpPtr = evtPtr.evtParams[0];
                        short oiList = ((PARAM_OBJECT) evpPtr).oiList;

                        HashSet<Pair> uniquePairs = new HashSet<>();

                        for (o = qual_GetFirstOiList(evtPtr.evtOiList); o != -1; o = qual_GetNextOiList()) {
                            short oi2 = localOiList[o].oilOi;
                            if (oi1 == oi2) {
                                flag = 0;
                                colList = make_ColList2(colList, oi1, oiList, uniquePairs);
                            }
                        }
                        ...
                        for (o = qual_GetFirstOiList(oiList); o != -1; o = qual_GetNextOiList()) {
                            short oi2 = localOiList[o].oilOi;
                            if (oi1 == oi2) {
                                colList = make_ColList2(colList, oi1, evtPtr.evtOiList, uniquePairs);
                            }
                        }
                        break;
                    ...
                    case (-12 << 16): // CNDL_EXTOUTPLAYFIELD:
                        ...
                        // fall-through intentionally omitted to remove redundant case label
        ...
        //System.out.println("time make_collist_1 is:"+(System.nanoTime()-starttime));
        return colList;
    }

    int make_ColList2(int colList, short oi1, short ol, HashSet<Pair> uniquePairs) {
        //long starttime = System.nanoTime();
        CObjInfo[] localOiList = rhPtr.rhOiList;
        short o;
        ...
        //System.out.println("time make_collist_2 is:"+(System.nanoTime()-starttime));
        return colList;
    }

    // Helper class to represent a pair of oi1 and oi2
    class Pair {
        private final short oi1;
        private final short oi2;

        public Pair(short oi1, short oi2) {
            this.oi1 = oi1;
            this.oi2 = oi2;
        }
        ...
        public boolean equals(Object obj) {
            ...
            Pair other = (Pair) obj;
            return oi1 == other.oi1 && oi2 == other.oi2;
        }
    }
    ...
    public int getCollisionFlags() {
        ...
        for (CEventGroup evgPtr : events) {
            if (evgPtr == null)
                continue;

            int evgPtrSize = evgPtr.evgNAct + evgPtr.evgNCond;
            for (CEvent evtPtr : evgPtr.evgEvents) {
                if (evtPtr == null)
                    continue;

                if (evtPtr.evtNParams > 0) {
                    for (CParam evpPtr : evtPtr.evtParams) {
                        if (evpPtr != null && evpPtr.code == 43) { // PARAM_PASTE
                            PARAM_SHORT p = (PARAM_SHORT) evpPtr;
                            int value = p.value;
                            flag |= value == 1 ? CColMask.CM_OBSTACLE : (value == 2 ? CColMask.CM_PLATFORM : 0);
                        }
                    }
                }
            }
    ...
    public void endForEach()
    {
        ...
        while(true)
        {
			...
			int nCondition = (-41 << 16);	// CNDL_EXTONLOOP;
			int nCondition2 = (-41 << 16);	// CNDL_EXTONLOOP;
			Map <String, Integer> conditionIndexMap = null;
			if (pForEach.number > 0)
			{
				CObject pHo = pForEach.objects[0];
				conditionIndexMap = (pHo.hoType == COI.OBJ_SPR) ? onEachLoopConditionIndexesActives : onEachLoopConditionIndexes;
			}
			if (conditionIndexMap != null)
			{
				Integer idxValue = conditionIndexMap.get(pForEach.name);
				if (idxValue != null)
					nCondition = -idxValue.intValue() * 65536;
				if (pForEach2 != null)
				{
					conditionIndexMap = null;
					if (pForEach2.number > 0)
					{
						CObject pHo2 = pForEach2.objects[0];
						conditionIndexMap = (pHo2.hoType == COI.OBJ_SPR) ? onEachLoopConditionIndexesActives : onEachLoopConditionIndexes;
					}
					if (conditionIndexMap != null)
					{
						idxValue = conditionIndexMap.get(pForEach2.name);
						if (idxValue != null)
							nCondition2 = -idxValue.intValue() * 65536;
					}
				}
			}
    ...
    public void addForEach(String pName, CObject pHo, short oi)
    {
        ...
        while(pForEach!=null)
        {
            if (pForEach.oi==oi && pName.equalsIgnoreCase(pForEach.name))
            {
