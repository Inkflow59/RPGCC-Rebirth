...
public class CImageBank implements IEnum
{
    ...
    public boolean removeImageWithHandle(short handle)
    {
    ...
    public void load() {
        final Iterator<BankSlot> i = images.iterator();
    ...
    public void loadImageList(short[] handles, boolean antialias_flag)
    {
    	...
    	for (int i = 0; i < handles_length; ++ i)
        {
            ...
            synchronized (slot) {
                slot.useCount++;
	            if (slot.texture == null && slot.dataOffset != -1 || slot.texture.getResampling() != antialias_flag)
	            {
	                file.seek (slot.dataOffset);
	                //Log.v("MMFRuntime", "Updating texture for handle: "+handle+" antialias: "+(antialiased_flag?" yes":"no"));
	                slot.antialiased = antialias_flag;
	                slot.texture = new BankImage (slot, file);
	                if(slot.texture != null)
	                    slot.texture.setResampling(antialias_flag);
	            }
            }
        }
    }

    public void loadImageByHandle(short handle, boolean antialias_flag) {
    ...
    public CImageInfo getImageInfoEx(short nImage, float nAngle, float fScaleX, float fScaleY)
    {
        ...
        BankSlot slot = images.get (nImage);
        CImage image = slot.texture;
        if(image == null)
        {
            if(slot != null && slot.useCount != 0)
            {
               slot.uploadTexture();
                image = slot.texture;
                if(image == null)
