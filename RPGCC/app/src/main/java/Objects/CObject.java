...
public abstract class CObject implements IDrawable
{
	...
	{
		if(hoEffect != null)
			roc.rcChanged =hoEffect.updateParamTexture();
	}
	...
	{
		if(!hoAdRunHeader.rhApp.effectBank.isEmpty())
		{
			if (hoEffect == null) {

				hoEffect = new CEffectEx(hoAdRunHeader.rhApp);
				if (hoEffect.initialize(index, rgba)) // Needs a handle (from objects)??
				{
					fillEffectData();
	...
	public int checkOrCreateEffectIfNeeded(String name, int rgba)
	{
		if(!hoAdRunHeader.rhApp.effectBank.isEmpty())
		{
			if (hoEffect == null) {
				...
				hoEffect = new CEffectEx(hoAdRunHeader.rhApp);
				if (hoEffect.initialize(name, rgba))
					return hoEffect.getIndexShader();
				else
					return -1;

			} else if (hoEffect.getName().contains(name))
				return hoEffect.getIndexShader();
			else {
				hoEffect.destroy();
	...
	{
		COI oiPtr = hoAdRunHeader.rhApp.OIList.getOIFromHandle(hoOi);
		if(oiPtr.oiEffectOffset != 0
				&& hoEffect != null
					&& oiPtr.oiEffectData != null)
