...
public class CSprite 
{
	...
	public int sprEffectShader;			/// if this value is -1 means a default shader
	...
	public CSprite(CImageBank b) 
	{
		...
		sprEffectShader = -1;	// very sensitive just put as default for now
	}
		...
		synchronized(ci)
		{
			synchronized(GLRenderer.inst) 
			{
				...
				if(sprEffectShader != - 1) {
					GLRenderer.inst.setEffectShader(sprEffectShader);
					if((sprFlags & CSprite.SF_REAF) == CSprite.SF_REAF )
					{
						GLRenderer.inst.updateVariable1f("fPixelWidth", 1.0f/(float)(sprX2 - sprX1));
						GLRenderer.inst.updateVariable1f("fPixelHeight", 1.0f/(float)(sprY2 - sprY1));
					}
				}
				else
				...
				if ( (sprFlags&CSprite.SF_NOHOTSPOT) == 0 )
					bHotSpot = 1;
				GLRenderer.inst.renderScaledRotatedImage2(ci, (MMFRuntime.inst.app.hdr2Options & CRunApp.AH2OPT_ANTIALIASED) != 0, sprAngle, sprScaleX, sprScaleY, bHotSpot, sprX, sprY, sprEffect, sprEffectParam);
