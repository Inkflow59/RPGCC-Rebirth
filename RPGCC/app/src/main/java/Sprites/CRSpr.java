...
public class CRSpr 
{
    ...
    public int rsEffectShader;			/// if this value is -1 means a default shader
    ...
    public void init1(CObject ho, CObjectCommon ocPtr, CCreateObjectInfo cobPtr)
    {
        ...
        rsEffectShader = -1;
        if((rsEffect & GLRenderer.BOP_MASK) == GLRenderer.BOP_EFFECTEX)
        {
            rsEffectShader = hoPtr.checkOrCreateEffectIfNeeded(hoPtr.hoOiList.oilIndexEffect, hoPtr.hoOiList.oilEffectParam);
        }
	...
	// --------------
		if ((hoPtr.hoOEFlags&CObjectCommon.OEFLAG_ANIMATIONS)!=0)
		{
            ...
            if (pSpr!=null)
            {
                ...
                spriteGen.modifSpriteEffect(pSpr, rsEffect, rsEffectParam, rsEffectShader);
    ...
    public void kill(boolean fast)
    {
        ...
        if(hoPtr.ros.rsEffectShader != -1)
            GLRenderer.inst.removeShader(hoPtr.ros.rsEffectShader);
    }
