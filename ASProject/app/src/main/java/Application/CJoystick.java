...
public class CJoystick implements ITouchAware
{
	...
	private static double COMFORT_FACTOR;
	private static double COMFORT_FACTOR_DPAD = 1.15;
	private static double COMFORT_FACTOR_JPAD = 1.00;
	...
	public CJoystick(CRunApp app, int flags)
	{
		...
		double half_inch_ratio = (nDensityAvg/2.0)/80.0;

		if(Size < 5.1)
			this.nScale = (float)(0.6 * half_inch_ratio);
		else if(Size > 5.1 && Size < 6.6)
			this.nScale = (float)(0.8 * half_inch_ratio);
		else
			this.nScale = (float)(1.2 * half_inch_ratio);
	...
	public void draw()
	{
		...
		if ((flags&JFLAG_JOYSTICK)!=0)
		{
			if ((flags&JFLAG_DPAD)!=0)
			{
				if ( (joystick & 1) != 0 )
				{
					...
					renderer.renderImage(joyUpD, bAntialias, imagesX[KEY_JOYSTICK]-joyUp_width/2, imagesY[KEY_JOYSTICK]-joyUp_height, joyUp_width, joyUp_height, 0, 0);
				}
				else
				{
					//joyUp.setResampling(bAntialias);
					renderer.renderImage(joyUp, bAntialias,imagesX[KEY_JOYSTICK]-joyUp_width/2, imagesY[KEY_JOYSTICK]-joyUp_height, joyUp_width, joyUp_height, 0, 0);
				}
				if ( (joystick & 2) != 0 )
				{
					//joyDownD.setResampling(bAntialias);
					renderer.renderImage(joyDownD, bAntialias,imagesX[KEY_JOYSTICK]-joyDown_width/2, imagesY[KEY_JOYSTICK], joyDown_width, joyDown_height, 0, 0);
				}
				else
				{
					//joyDown.setResampling(bAntialias);
					renderer.renderImage(joyDown, bAntialias,imagesX[KEY_JOYSTICK]-joyDown_width/2, imagesY[KEY_JOYSTICK], joyDown_width, joyDown_height, 0, 0);
				}
				if ( (joystick & 4) != 0 )
				{
					//joyLeftD.setResampling(bAntialias);
					renderer.renderImage(joyLeftD, bAntialias,imagesX[KEY_JOYSTICK]-joyLeft_width, imagesY[KEY_JOYSTICK]-joyLeft_height/2, joyLeft_width, joyLeft_height, 0, 0);
				}
				else
				{
					//joyLeft.setResampling(bAntialias);
					renderer.renderImage(joyLeft, bAntialias,imagesX[KEY_JOYSTICK]-joyLeft_width, imagesY[KEY_JOYSTICK]-joyLeft_height/2, joyLeft_width, joyLeft_height, 0, 0);
				}
				if ( (joystick & 8) != 0 )
				{
					//joyRightD.setResampling(bAntialias);
					renderer.renderImage(joyRightD, bAntialias, imagesX[KEY_JOYSTICK], imagesY[KEY_JOYSTICK]-joyLeft_height/2, joyRight_width, joyRight_height, 0, 0);
				}
				else
				{
					//joyRight.setResampling(bAntialias);
					renderer.renderImage(joyRight, bAntialias, imagesX[KEY_JOYSTICK], imagesY[KEY_JOYSTICK]-joyLeft_height/2, joyRight_width, joyRight_height, 0, 0);
				}
			}
			else
			{
				//joyBack.setResampling(bAntialias);
				renderer.renderImage(joyBack, bAntialias,imagesX[KEY_JOYSTICK]-joyBack_width/2, imagesY[KEY_JOYSTICK]-joyBack_height/2, joyBack_width, joyBack_height, 0, 0);
				//joyFront.setResampling(bAntialias);
				renderer.renderImage(joyFront, bAntialias,imagesX[KEY_JOYSTICK]+joystickX-joyFront_width/2, imagesY[KEY_JOYSTICK]+joystickY-joyFront_height/2, joyFront_width, joyFront_height, 0, 0);
			}
		}
		if ((flags&JFLAG_FIRE1)!=0)
		{
			...
			renderer.renderImage(tex, bAntialias, imagesX[KEY_FIRE1]-tw/2, imagesY[KEY_FIRE1]-th/2, tw, th, 0, 0);
		}
		if ((flags&JFLAG_FIRE2)!=0)
		{
			...
			//tex.setResampling(bAntialias);
			renderer.renderImage(tex, bAntialias, imagesX[KEY_FIRE2]-tw/2, imagesY[KEY_FIRE2]-th/2, tw, th, 0, 0);
		}
