...
//    public static final int LEF_RESERVED_2=0x2000;
    ...
    public static final int  ANDROIDFOPT_HOLDUNTILSOUNDSLOADED = 0x0400;
    ...
    public boolean loadFullFrame(int index)
    {
		...
		while (chk.chID!=0x7F7F)	// CHUNK_LAST
		{
            ...
            case  0x3349:
                ...
                if (number_params > 0)
                {
                    effectData = new int[number_params];
			...
			case 0x334A:
                ...
                if((iPhoneOptions & ANDROIDFOPT_HOLDUNTILSOUNDSLOADED) != 0)
                    app.WAIT4ALL = true;
                else
                    app.WAIT4ALL = false;
        ...
        if ((app.hdr2Options2 & CRunApp.AH2OPT2_WINDOWSLIKEBOXCOLLISIONS) != 0) {
            for (n = 0; n < app.OIList.oiMaxIndex; n++) {
                COI poi = app.OIList.ois[n];
                if (poi != null && poi.oiOC != null && poi.oiType == COI.OBJ_BKD) {
                    COCBackground poc = ((COCBackground)poi.oiOC);
                    short v = poc.ocObstacleType;
                    if (poc.ocColMode != 0 && (v == COC.OBSTACLE_SOLID || v == COC.OBSTACLE_PLATFORM)) {
                        poc.ocColMode = 0;
                        short imgHandle = poc.ocImage;
                        CImage img = app.imageBank.getImageFromHandle(imgHandle);
                        if (img != null)
							img.setOpaqueMask(true);
                    }
                }
            }
        }

        if((effect & GLRenderer.EFFECT_MASK) == GLRenderer.BOP_EFFECTEX)
            createFrameEffect();
    ...
    public void initSize() {
        if ((app.gaFlags & CRunApp.GA_STRETCH) != 0
	...
	{
        int start_block = app.file.getFilePointer();
		int n;
        ...
        for (n=0; n<nLayers; n++)
        {
            ...
            if(number_params > 0)
            {
                layers[n].effectData = new int[number_params];
    ...
    public int checkOrCreateEffectIfNeeded(int index, int rgba)
    {
        if(!app.effectBank.isEmpty())
        {
            if (effectEx == null)
            {
                ...
                effectEx = new CEffectEx(app);
                if (effectEx.initialize(index, rgba))
                {
                    if(effectData != null)
                        effectEx.setEffectData(effectData);
    ...
    public int checkOrCreateEffectIfNeeded(String name, int rgba)
    {
        if(!app.effectBank.isEmpty())
        {
            if (effectEx == null) {
                ...
                effectEx = new CEffectEx(app);
                if (effectEx.initialize(name, rgba))
                    return effectEx.getIndexShader();
                else
                    return -1;
            }
