...
//			getHolder().setFormat(PixelFormat.TRANSLUCENT);
		...
		hasType = (Build.VERSION.SDK_INT > 17);
	...
	public boolean swapBuffers ()
	{
		...
		if (!egl.eglSwapBuffers(eglDisplay, eglSurface)) {
			...
			if(eglError == EGL10.EGL_BAD_SURFACE)
				createSurface(SurfaceView.inst.getHolder());
			return false;
		}
		return true;
	}
	...
	public void surfaceCreated (SurfaceHolder holder)
	{
		...
		Log.Log("Shader info: "+ GLES20.glGetString(GLES20.GL_SHADING_LANGUAGE_VERSION) );
	...
	public void startEGLContext(SurfaceHolder holder) {
		...
		hasUiContext = !eglContext.equals(EGL10.EGL_NO_CONTEXT);

		Log.Log ("hasUiContext: " + hasUiContext);
		Log.Log("Context: "+(verifyContext() ? "valid":"lost"));
	...
	public Bitmap drawBitmap() {
		...
		return drawBitmap(0, 0, w, h);
	}

	public Bitmap drawBitmap(int x, int y, int w, int h) {
		Bitmap bitmap = null;

		if(SurfaceView.inst != null && Build.VERSION.SDK_INT < 17)
	...
	public boolean onKeyDown(final int keyCode, KeyEvent msg)
	{
		...
		if(MMFRuntime.OUYA && MMFRuntime.inst.app.ouyaObjects.size() != 0)
		{
			...
			if((jplayer = CRunOUYA.keyDownAtPlayerNumber(keyCode, msg)) != -1)
			{
				...
				byte b = (byte)getPlayerBits(keyCode);
				rhPlayer[jplayer] |= b;
		...
		if(MMFRuntime.NEXUSTV && MMFRuntime.inst.app.nexustvObjects.size() != 0)
		{
			...
			{
//				byte pByte = CJoystickNEXUSTV.rhPlayer[jplayer];
//				String sp = "00000000"+Integer.toBinaryString(pByte);
				byte b = (byte)(getPlayerBits(keyCode));
//				String s = "00000000"+Integer.toBinaryString(b);
				CJoystickNEXUSTV.rhPlayer[jplayer] |= b;
//				String out = "00000000"+Integer.toBinaryString(CJoystickNEXUSTV.rhPlayer[jplayer]);
//				Log.Log("Player: "+jplayer+" result in down: "+ s.substring(s.length()-8)
//						+ " came with ("+pByte+"): "+sp.substring(sp.length()-8)
//						+" and leave: "+out.substring(out.length()-8)
//						+" Array: "+Arrays.toString(CJoystickNEXUSTV.rhPlayer));
...
//				String s = Integer.toBinaryString(b);
				CJoystickNEXUSTV.rhPlayer[jplayer] &= b;
//				String out = "11111111"+Integer.toBinaryString(CJoystickNEXUSTV.rhPlayer[jplayer]);
//				Log.Log("Player: "+jplayer+" result in   up: "+ s.substring(s.length()-8)
//						+ " came with ("+pByte+"): "+sp.substring(sp.length()-8)
//						+" and leave: "+out.substring(out.length()-8)
//						+" Array: "+ Arrays.toString(CJoystickNEXUSTV.rhPlayer));
				if(!MMFRuntime.inst.app.nexustvObjects.isEmpty()) {
			...
			{
				Log.Log("Player incorrectly detected ...");
			}
		}
	...
	public boolean onGenericMotionEvent(MotionEvent event)
	{
		...
		if(MMFRuntime.OUYA) {
			...
			if(!MMFRuntime.inst.app.ouyaObjects.isEmpty() && Build.VERSION.SDK_INT > 11
			...
			if(Build.VERSION.SDK_INT > 11
				...
				if ((event.getSource() & InputDevice.SOURCE_CLASS_JOYSTICK) != 0) {
					...
					if (axisX * axisX + axisY * axisY > (0.20f * 0.20f)) {
						...
						CJoystickFIRETV.rhPlayer[OuyaPlayer] |= (byte)joystick;
					}
					else
					{
						CJoystickFIRETV.rhPlayer[OuyaPlayer] &= 0xF0;
					}
				}
		...
		if(MMFRuntime.NEXUSTV) {
			...
			if(Build.VERSION.SDK_INT > 11
				...
				if((event.getSource() & InputDevice.SOURCE_JOYSTICK) ==
					...
					if (axisX * axisX + axisY * axisY > (0.15f * 0.15f))
					{
						double angle = (2*joyPI + Math.atan2(-axisY, axisX))%(2*joyPI);
						//Log.Log("X: "+axisX+" Y: "+axisY+" joystick: "+joystick+" Angle: "+angle*180.0/joyPI);
						joystick = joystickResult(angle);
						CJoystickFIRETV.rhPlayer[nexusPlayer] |= (byte)joystick;
					}
					else
					{
						CJoystickFIRETV.rhPlayer[nexusPlayer] &= 0xF0;
					}
				}
			...
			if(Build.VERSION.SDK_INT > 11
				...
				if((event.getSource() & InputDevice.SOURCE_JOYSTICK) ==
					...
					if (axisX * axisX + axisY * axisY > (0.15f * 0.15f))
					{
						...
						//Log.Log("X: "+axisX+" Y: "+axisY+" joystick: "+joystick+" Angle: "+angle*180.0/joyPI);
						joystick = joystickResult(angle);
						CJoystickFIRETV.rhPlayer[firePlayer] |= (byte)joystick;
					}
					else
					{
						CJoystickFIRETV.rhPlayer[firePlayer] &= 0xF0;
					}
				}
	...
	public boolean onTrackballEvent(final MotionEvent event)
	{
		if (MMFRuntime.inst.app != null) {
			...
			return MMFRuntime.inst.touchManager.process(event);
		}
		return false;
	}
	...
	public boolean onTouchEvent(final MotionEvent event)
	{
		if (MMFRuntime.inst.app != null && MMFRuntime.inst.touchManager != null) {
			if((!MMFRuntime.OUYA && !MMFRuntime.FIRETV)
					|| (MMFRuntime.OUYA && OuyaPlayer == MMFRuntime.inst.OuyaTouchPad)
					|| (MMFRuntime.FIRETV))
				if(MMFRuntime.inst.app.appRunningState > CRunApp.SL_RESTART && MMFRuntime.inst.app.appRunningState < CRunApp.SL_QUIT)
					return MMFRuntime.inst.touchManager.process(event);
				else
	...
	public class CTEGLConfigChooser extends ComponentSizeChooser {
		...
		public CTEGLConfigChooser(int mode, int version, boolean withDepthBuffer) {
			super(version,  (mode==MODE_565 ? 5: (mode==MODE_888 ? 8:4)), 	//Red_size
							(mode==MODE_565 ? 6: (mode==MODE_888 ? 8:4)),	//Green_size
							(mode==MODE_565 ? 5: (mode==MODE_888 ? 8:4)),	//Blue_size
							(mode==MODE_565 ? 0: (mode==MODE_888 ? 8:4)), 	//Alpha_size //(ES==2?0:0)
					withDepthBuffer ? 16 : 0, //DepthBuffer
					0); //Stencil
		}
	}
