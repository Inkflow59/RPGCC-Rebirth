...
public class ES2Renderer extends GLRenderer
{
	...
	@Override
	public native void setBlendMode (boolean on_mode);
	@Override
	public native void setCurrentView(int currentWidth, int currentHeight, float scaleX, float scaleY);
	...
	public void beginWholeScreenDraw ()
	{
		...
		setTransformation(1.0f, 1.0f, 0,0,0,0, 0.0f);
	...
	@Override
	public native void setTransformation (float scalex, float scaley, int posx, int posy, int hotspotx, int hotspoty, float angle);

	public native void setTransformationMatrix (float[] array);

	public native void resetTransformation ();
	...
	(ITexture image, boolean resampling, int x, int y, int w, int h, int inkEffect, int inkEffectParam);

	@Override
	public native void renderScaledRotatedImage
	(ITexture image, boolean resampling, float angle, float sX, float sY, int hX, int hY,
     int x, int y, int w, int h, int inkEffect, int inkEffectParam);

	@Override
	public native void renderScaledRotatedImage2
	(ITexture image, boolean resampling, float angle, float sX, float sY,
	...
	@Override
	public native void setSurfaceTextureAtIndex(ITexture image, String var_name, int index);

	@Override
	public native void updateSurfaceTexture();
	...
	@Override
	public native void setBackgroundUse(int shaderIndex);
	...
	public Bitmap drawBitmap(int x, int y, int w, int h, Bitmap.Config config) {
		...
		GLES20.glGetError();
		GLES20.glFinish();
		GLES20.glReadPixels(0, 0, screenwidth, screenheight, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, bb);
		int gl_error = GLES20.glGetError();
		if(gl_error != GLES20.GL_NO_ERROR)
			Log.Log("Error: "+ Integer.toHexString(gl_error));
		bb.rewind();
	...
	@Override
	public void renderBlitFull(ITexture image, boolean resampling) {
		renderImage(image, resampling, 0, 0, image.getWidth(), image.getHeight(), 0, 0);
	}
