...
public abstract class COC implements IDrawable
{
    ...
    */
    public int checkOrCreateEffectIfNeeded(@NonNull CRunApp app, COI oi)
    {
        if(!app.effectBank.isEmpty()) {
            if (ocEffect == null) {
                ocEffect = new CEffectEx(app);
                if (ocEffect.initialize(oi.oiIndexEffect, oi.oiInkEffectParam))
                {
                    fillEffectData(oi);
    ...
    public void fillEffectData(COI oiPtr)
    {
        if(oiPtr.oiEffectOffset != 0 && ocEffect != null && oiPtr.oiEffectData != null)
        {
