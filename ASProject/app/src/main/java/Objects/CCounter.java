	...
	public void getZoneInfos()
    {
        ...
        switch (adCta.odDisplayType)
        {
            ...
            case 5:	    // CTA_TEXT:
                ...
                rTemp.copyRect (rc);
                textSurface.measureText (s, dtflags, font.getFontInfo(), rTemp, -1);
                ht = rTemp.bottom - rTemp.top;
                rc.right = x2;	// keep zone width
                if (ht != 0)
                {
                    hoImgXSpot = hoImgWidth = (short) (rTemp.right - rTemp.left);
                    if (hoImgHeight < ht) //(rc.bottom - rc.top))
                    {
                        hoImgHeight = (short) ht; //(rc.bottom - rc.top);
                    }
	...
	public void draw()
    {
        ...
        switch (adCta.odDisplayType)
        {
            case 4:	    // CTA_ANIM:
                ...
                hoAdRunHeader.spriteGen.pasteSpriteEffect(adCta.frames[rsOldFrame], hoRect.left, hoRect.top, 0, effect, effectParam, effectShader);
                break;
            ...
            case 1:	    // CTA_DIGITS:
                ...
                for (i = 0; i < s.length(); i++)
                {
                    ...
                    if(img != -1)
                    {
                        hoAdRunHeader.spriteGen.pasteSpriteEffect(img, x, y, 0, effect, effectParam, effectShader);
                        CImage ifo = hoAdRunHeader.rhApp.imageBank.getImageFromHandle(img);
                        if(ifo != null)
                            x += ifo.getWidth();
                    }
