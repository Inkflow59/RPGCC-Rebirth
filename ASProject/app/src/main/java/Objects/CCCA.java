...
public class CCCA extends CObject
{
    ...
    void startCCA(String filename, CObjectCommon ocPtr, boolean bInit, int nStartFrame)
    {
        ...
        subApp.gaFlags    = hoAdRunHeader.rhApp.gaFlags;
        subApp.gaNewFlags = hoAdRunHeader.rhApp.gaNewFlags;
  ...
  /*  public void sendKey(int keyCode, boolean bState)
    ...
    public void reinitDisplay() {
        ...
        if (subApp != null && subApp.run != null && subApp.run.rhNObjects != 0)
        {
            int cptObject = subApp.run.rhNObjects;
            for(CObject hoPtr : subApp.run.rhObjectList) {
                if(hoPtr != null) {
                    hoPtr.reinitDisplay();
                    -- cptObject;
                    if(cptObject == 0)
