...
public void handle()
    ...
    public void loadImageByHandle(short handle, boolean bAntialias)
    {
        short img_handle = hoAdRunHeader.rhApp.imageBank.enumerate(handle);
        if(img_handle != -1)
            hoAdRunHeader.rhApp.imageBank.loadImageByHandle(handle, bAntialias);
    }

    public void loadImageByHandle(short handle)
    {
        //Reading anti-aliased flag from object
        hoAdRunHeader.rhApp.imageBank.loadImageByHandle(handle, hoOiList.oilAntialias);
    }
    ...
    public void generateEvent(int code, int param)
    {
	    ...
	    {
	        if (hoAdRunHeader.rh2PauseCompteur == 0 && hoAdRunHeader.rhEvtProg.bReady)
	        {
...
/*    public void addBackdrop(Bitmap img, int x, int y, int dwEffect, int dwEffectParam, int typeObst, int nLayer)
    ...
    {
        CRunApp.HFile hfile = hoAdRunHeader.rhApp.openHFile(path);
        return hfile;
    }
