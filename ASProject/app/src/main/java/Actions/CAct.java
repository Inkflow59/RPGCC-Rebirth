...
//              act = new ACT_SETSAMPLEFREQ();
                break;
			case ((36 << 16) | 0xFFFE):		// 	ACT_PLAYSAMPLE2
...
//              act = new ACT_SETFRAMEHEIGHT();
			...
			case ((33<<16)|0xFFFD):			//  ACT_SETFRAMEEFFECTPARAM
				act=new ACT_SETFRAMEEFFECTPARAM();
				break;
			case ((34<<16)|0xFFFD):			//  ACT_SETFRAMEEFFECTPARAMTEXTURE		
				act=new ACT_SETFRAMEEFFECTPARAMTEXTURE();
				break;
...
//              act = new ACT_CREATEBYNAME();
                break;
            case ((2 << 16) | 0xFFFB):			//  ACT_CREATEEXP
//              act = new ACT_CREATEEXP();
                break;
            case ((3 << 16) | 0xFFFB):			//  ACT_CREATEBYNAMEEXP
...
//                      act = new ACT_EXTSTOPTORQUE();
        ...
        if (act != null)
        {
			...
			if (bExtra) {
				act.evtNParams--;
				int paramstart = app.file.getFilePointer();
				short paramsize = app.file.readAShort();
				short paramcode = app.file.readAShort();      // PARAM_SUBACTION
        ...
        {
            Log.Log("Missing action: " + Integer.toHexString(c));
        }
