...
public class CRun
{
	...
	public int rhLastCreatedObjectFixedValue;
...
//			rhOiList[n] = null;
//		}
		Arrays.fill(rhOiList, 0, rhMaxOI, null);
		// Random generator
	...
	public void f_ObjMem_Init()
	{
		...
		//	rhObjectList[i] = null;
		//}
		Arrays.fill(rhObjectList, 0, rhMaxObjects, null);
	}
	...
	public int prepareFrame()
	{
		...
		for (oiPtr = rhApp.OIList.getFirstOI(); oiPtr != null; oiPtr = rhApp.OIList.getNextOI())
		{
			...
			if (type >= COI.OBJ_SPR)
			{
                ...
                rhOiList[count].oilIndex = (short)count;
		...
		rhEvtProg.prepareQualifierList(this.rhOiList);
		...
		for (i = 0; i < nLayers; i++)
		{
			...
			for (j = 0; j < nLOs; j++)
			{
				...
				for (int ns = 0; ns < 4; ns++)
				{
					...
					{
						if(bDeleteFrame && plo.loSpr[ns].sprEffectShader != -1)
							GLRenderer.inst.removeShader(plo.loSpr[ns].sprEffectShader);
						spriteGen.delSpriteFast(plo.loSpr[ns]);
						plo.loSpr[ns] = null;
					}
				}
				COI poi = rhApp.OIList.getOIFromHandle(plo.loOiHandle);
				if(bDeleteFrame && poi.oiOC.ocEffect != null)
					poi.oiOC.ocEffect.destroy();
			}
			...
			if (pLayer.pBkd2 != null)
			{
				...
				for (j = 0; j < pLayer_pBkd2_size; j++)
				{
					...
					for (int ns = 0; ns < 4; ns++)
					{
						...
						{
							if(bDeleteFrame && pbkd.pSpr[ns].sprEffectShader != -1)
								GLRenderer.inst.removeShader(pbkd.pSpr[ns].sprEffectShader);
							spriteGen.delSpriteFast(pbkd.pSpr[ns]);
							pbkd.pSpr[ns] = null;
						}
					}
					if(pbkd.bkdEffect != null)
						pbkd.bkdEffect.destroy();
				}
			}
			...
			if(bDeleteFrame && (pLayer.effect & GLRenderer.EFFECT_MASK) == GLRenderer.BOP_EFFECTEX)
			{
				if(pLayer.effectEx != null)
					pLayer.effectEx.destroy();
				pLayer.effectEx = null;
			}
		}
	}
	...
	public int f_CreateObject(short hlo, short oi, int coordX, int coordY, int initDir, short flags, int nLayer, int numCreation)
	{
		while (true)
		{
			...
			if ( oilPtr.oilRealQualifiers != null )
			{
				...
				for(short qualNum : oilPtr.oilRealQualifiers)
				{
					...
					int firstFreePos = currentOisPerQualifier.qoiList.length;
            ...
            rhLastCreatedObjectFixedValue = (hoPtr.hoCreationId << 16) | (hoPtr.hoNumber & 0xFFFF);
	...
	public void f_KillObject(int nObject, boolean bFast)
	{
		...
		if ( poil != null && poil.oilObject == (short)0x8000 )		// last object instance?
		{
			...
			if(hoPtr.hoEffect != null && hoPtr.ros.rsEffectShader != -1) {
				hoPtr.hoEffect.destroy();
				hoPtr.hoEffect = null;
			}
		...
		if (rh2PauseCompteur == 1)
		{
			...
			for(CObject hoPtr : localObjectList) {
				if (hoPtr == null)
					continue;
				--count;
				if(hoPtr.hoType == COI.OBJ_CCA && MMFRuntime.inst.isOnPause) {
					CCCA ccca = (CCCA)hoPtr;
					ccca.pause();
				}
				if(hoPtr.hoType >= COI.KPX_BASE)
				{
					CExtension e = (CExtension) hoPtr;
					if(e != null && e.ext != null)
	...
	public void updateBckgEffectTexture()
	{
		...
		for(;nLayer<rhFrame.nLayers;nLayer++)
		{
			...
			for (int i=0; i<nLOs; i++)
			{
				...
				if(oiPtr != null && oiPtr.oiOC != null && oiPtr.oiOC.ocEffect != null)
				{
					if(oiPtr.oiOC.ocEffect.updateParamTexture())
						oiPtr.oiOC.ocEffect.refreshParamSurface();
				}
			}
		}
	...
	public void redrawLevel(int flags)
	{
		...
		rc.left = rc.top = 0;
		rc.right = Math.max((int)rhApp.gaCxWin, rhFrame.leEditWinWidth);
		rc.bottom = Math.max((int)rhApp.gaCyWin, rhFrame.leEditWinHeight);
		lgEdit = rc.right;
		x2edit = lgEdit - 1;
		htEdit = rc.bottom;
		y2edit = htEdit - 1;
			...
			if ( pLayer != null && (pLayer.dwOptions & CLayer.FLOPT_TOHIDE) != 0 )
			{
				...
				for (i=0; i<nLOs; i++)
				{
					...
					for(CSprite spr : plo.loSpr) {
						if(spr != null) {
//							if(spr.sprEffectShader != -1)
//								GLRenderer.inst.removeShader(spr.sprEffectShader);
					...
					do
					{
						...
						if ( bWrap )
						{
							...
							case 0:
								...
								{
//									if(plo.loSpr[1].sprEffectShader != -1)
//										GLRenderer.inst.removeShader(plo.loSpr[1].sprEffectShader);
									spriteGen.delSprite(plo.loSpr[1]);
									plo.loSpr[1] = null;
								}
								if ( (dwWrapFlags & WRAP_Y) == 0 && plo.loSpr[2] != null )
								{
//									if(plo.loSpr[2].sprEffectShader != -1)
//										GLRenderer.inst.removeShader(plo.loSpr[2].sprEffectShader);
									spriteGen.delSprite(plo.loSpr[2]);
									plo.loSpr[2] = null;
								}
								if ( (dwWrapFlags & WRAP_XY) == 0 && plo.loSpr[3] != null )
								{
//									if(plo.loSpr[3].sprEffectShader != -1)
//										GLRenderer.inst.removeShader(plo.loSpr[3].sprEffectShader);
						...
						// Display object?
						//Log.Log("nLOs: "+i+" inside: "+((rc.left <= x2edit && rc.top <= y2edit && rc.right >= 0 && rc.bottom >= 0) ? "yes":"no"));
						...
						if ( rc.left <= x2edit && rc.top <= y2edit && rc.right >= 0 && rc.bottom >= 0 )
						{
							...
							if ( nLayer > 0 || !bLayer0Invisible)
							{
								...
								if ( plo.loSpr[nSprite] == null )
								{
									...
									{
										int shader = -1;
										if ((poi.oiInkEffect & GLRenderer.BOP_MASK) == GLRenderer.BOP_EFFECTEX)
											shader = poc.checkOrCreateEffectIfNeeded(rhApp, poi);
										...
										spriteGen.modifSpriteEffect(plo.loSpr[nSprite], poi.oiInkEffect, poi.oiInkEffectParam, shader);
									}
									break;
					...
					if ( bOut )
					{
						...
						{
//							if(plo.loSpr[nSprite].sprEffectShader != -1)
//								GLRenderer.inst.removeShader(plo.loSpr[nSprite].sprEffectShader);
	...
	public void screen_Update()
	{
		...
		synchronized(GLRenderer.inst)
		{
			GLRenderer.inst.fillZone (0, 0, rhApp.gaCxWin, rhApp.gaCyWin, background,
					GLRenderer.BOP_BLEND, (rhApp.frame.leFlags & CRunFrame.LEF_NOSURFACE )!=0 ? 128 : 0);
		}
		if (rh3Scrolling!=0)
		{
	...
	public void activeToBackdrop(CObject hoPtr, int nTypeObst, boolean bTrueObject)
	{
		...
		// Make a new program effect for this, active can be destroyed
		if(hoPtr.ros.rsEffectShader != -1)
		{
			COI poi = rhApp.OIList.getOIFromHandle(hoPtr.hoOi);
			toadd.bkdEffectShader = toadd.checkOrCreateEffectIfNeeded(rhApp, poi);
		}
		addBackdrop2(toadd);
	...
	public void addBackdrop2(CBkd2 toadd)
	{
						...
						&& (pbkd.inkEffect & CSpriteGen.EFFECT_MASK) == CSpriteGen.EFFECT_NONE)
				{
					...
					if (pbkd.inkEffect != toadd.inkEffect || pbkd.inkEffectParam != toadd.inkEffectParam)
					{
						...
						for (int j = 0; j < 4; j++)
						{
							if (pbkd.pSpr[j] != null)
							{
								spriteGen.modifSpriteEffect(pbkd.pSpr[j], pbkd.inkEffect, pbkd.inkEffectParam, pbkd.bkdEffectShader);
							}
						}
					}
					toadd = null;
		...
		boolean bLayerIsVisible = ((pLayer.dwOptions & (CLayer.FLOPT_TOHIDE | CLayer.FLOPT_VISIBLE)) == CLayer.FLOPT_VISIBLE);
		{
			boolean bSaveBkd = ((pLayer.dwOptions & CLayer.FLOPT_NOSAVEBKD) == 0);
			int dwWrapFlags = 0;
			int nSprite = 0;
			do
			{
				...
				if (bWrap)
				{
					...
					case 0:
						...
						{
//							if(pbkd.pSpr[1].sprEffectShader != -1)
//								GLRenderer.inst.removeShader(pbkd.pSpr[1].sprEffectShader);
							spriteGen.delSprite(pbkd.pSpr[1]);
							pbkd.pSpr[1] = null;
						}
						if ((dwWrapFlags & WRAP_Y) == 0 && pbkd.pSpr[2] != null)
						{
//							if(pbkd.pSpr[2].sprEffectShader != -1)
//								GLRenderer.inst.removeShader(pbkd.pSpr[2].sprEffectShader);
							spriteGen.delSprite(pbkd.pSpr[2]);
							pbkd.pSpr[2] = null;
						}
						if ((dwWrapFlags & WRAP_XY) == 0 && pbkd.pSpr[3] != null)
						{
//							if(pbkd.pSpr[3].sprEffectShader != -1)
//								GLRenderer.inst.removeShader(pbkd.pSpr[3].sprEffectShader);
				...
				if (bTotalColMask || (rc.left < x2edit  + COLMASK_XMARGIN + 32 && rc.top < y2edit + COLMASK_YMARGIN))
				{
					...
					// Display object?
					if (bLayerIsVisible && rc.left <= x2edit && rc.top <= y2edit && rc.right >= 0 && rc.bottom >= 0)
					{
						////////////////////////////////////////
						// Non-background layer => create sprite
						//Log.Log("yes entered");
						...
						spriteGen.modifSpriteEffect(pbkd.pSpr[nCurSprite], pbkd.inkEffect, pbkd.inkEffectParam, pbkd.bkdEffectShader);
					}
		...
		if (pLayer.pBkd2 != null)
		{
			...
			for (i = 0; i < pLayer_pBkd2_size; i++)
			{
				...
				for (int ns = 0; ns < 4; ns++)
				{
					if (pbkd.pSpr[ns] != null)
					{
						if(pbkd.pSpr[ns].sprEffectShader != -1)
							GLRenderer.inst.removeShader(pbkd.pSpr[ns].sprEffectShader);
		...
		if ((pLayer.dwOptions & CLayer.FLOPT_TOHIDE) == 0)
		{
			...
			for (i = 0; i < pLayer_pBkd2_size; i++)
			{
						...
						rc.right <= -COLMASK_XMARGIN - 32 || rc.bottom <= - COLMASK_YMARGIN))
				{
					...
					{
//						if(pbkd.pSpr[nCurSprite].sprEffectShader != -1)
//							GLRenderer.inst.removeShader(pbkd.pSpr[nCurSprite].sprEffectShader);
	...
	public static boolean compareTerFloat(float value1, float value2, short comparaison)
	{
		switch (comparaison)
		{
		case 0:	// COMPARE_EQ:
	...
	public int txtDoDisplay(CEvent pe, int txtNumber)
	{
		...
		{
			short noi = qoil.qoiList[count];
			if (noi < 0)
				break;
			txtDisplay(pe, noi, txtNumber);
			count += 2;
		}
		;
		return -1;
	...
	public void reinitDisplay ()
	{
		...
		{
			if(SurfaceView.inst.eglSurface == null)
