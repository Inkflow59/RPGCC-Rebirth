...
public class CSpriteGen {
	...
	public CSprite addOwnerDrawSprite(final int x1, final int y1, final int x2, final int y2,
		...
		if(extraInfo != null && extraInfo.ros != null)
		{
		...
		{
			ptSpr.sprEffect = CSprite.EFFECTFLAG_TRANSPARENT; // Adding Blend
			ptSpr.sprEffectParam = 0;
			ptSpr.sprEffectShader = -1;
		}
		ptSpr.sprScaleX = ptSpr.sprScaleY = 1.0f;		// ptSpr.sprTempScaleX = ptSpr.sprTempScaleY = 
	...
	// ------------------------------------------;
	public void modifSpriteEffect(CSprite ptSpr, final int effect, final int effectParam, final int effectShader) {
        ...
        if (ptSpr != null)
		{
			...
			if ((ptSpr.sprEffect & EFFECT_MASK) == GLRenderer.BOP_BLEND)
			{
				if(ptSpr.sprEffectParam == 128)
				{
					ptSpr.sprFlags |= CSprite.SF_DISABLED;
				}
				else if((ptSpr.sprFlags & CSprite.SF_DISABLED) == CSprite.SF_DISABLED)
				{
					ptSpr.sprFlags &= ~CSprite.SF_DISABLED;;
				}
			}

			if ((ptSpr.sprEffect & EFFECT_MASK) == GLRenderer.BOP_RGBAFILTER)
			{
	...
	*/
	...
	public void pasteSpriteEffect(short iNum, int iX, int iY, int flags, int effect, int effectParam, int effectShader) {
		int x1, y1;
		CImage ptei;
		...
		do {
			...
			if(effectShader != - 1)
			{
				...
				GLRenderer.inst.updateVariable1f("fPixelWidth",1.0f/(float)ptei.getWidth());
				GLRenderer.inst.updateVariable1f("fPixelHeight",1.0f/(float)ptei.getHeight());
			}
			else
				GLRenderer.inst.removeEffectShader();

			GLRenderer.inst.renderImage(ptei, (MMFRuntime.inst.app.hdr2Options & CRunApp.AH2OPT_ANTIALIASED) != 0, x1, y1, -1, -1, effect, effectParam);
	...
	public void winDrawSprites() {
		...
		boolean useResampling = ((MMFRuntime.inst.app.hdr2Options & CRunApp.AH2OPT_ANTIALIASED) != 0);
		CImage ci = null;
		int currentImage = -1;
		int currentShader = -1;
		//GLRenderer.inst.removeEffectShader();
		...
		for (ptSpr = ptFirstSpr; ptSpr != null; ptSpr = ptSpr.objNext)
		{
			...
			CLayer pLayer = frame.layers[ptSpr.sprLayer/2];
			if(pLayer != null && actLayer != ptSpr.sprLayer/2)
			{
				pLayer.calculateTransformation(MMFRuntime.inst.app);
				GLRenderer.inst.setTransformationMatrix(pLayer.transform);
				actLayer = ptSpr.sprLayer/2;
			}
			if (ptSpr.sprEffectShader != -1)
			{
				GLRenderer.inst.setEffectShader(ptSpr.sprEffectShader);
				if((ptSpr.sprFlags & CSprite.SF_REAF) == CSprite.SF_REAF)
				{
					GLRenderer.inst.updateVariable1f("fPixelWidth", 1.0f/(float)(ptSpr.sprX2- ptSpr.sprX1));
					GLRenderer.inst.updateVariable1f("fPixelHeight", 1.0f/(float)(ptSpr.sprY2- ptSpr.sprY1));
				}
			}
			// Sprite ownerdraw
			if ((ptSpr.sprFlags & CSprite.SF_OWNERDRAW) != 0)
			...
			// Normal sprite
				...
				// To avoid continuous shader change, integrate the draw method here and change the shader only when necessary
				//ptSpr.draw();

				if(ptSpr.sprX2 >= 0 && ptSpr.sprY2 >= 0 && ptSpr.sprY1 < GLRenderer.limitY && ptSpr.sprX1 < GLRenderer.limitX)
				{
					//final CImage ci=bank.getImageFromHandle(ptSpr.sprImg);
					if ( ptSpr.sprImg != currentImage )
					{
						currentImage = ptSpr.sprImg;
						ci = bank.getImageFromHandle(ptSpr.sprImg);
					}
					if(ci != null)
					{
						synchronized(GLRenderer.inst)
						{
							...
							//ci.setResampling(useResampling);
							//Log.Log("Draw "+(ptSpr.sprExtraInfo != null ? ptSpr.sprExtraInfo.hoOiList.oilName :"no active"));
							int bHotSpot = 0;
							if ( (ptSpr.sprFlags&CSprite.SF_NOHOTSPOT) == 0 )
								bHotSpot = 1;
							//Log.Log("Shader: "+ptSpr.sprEffectShader+" Image: "+ptSpr.sprImg);
							GLRenderer.inst.renderScaledRotatedImage2(ci, useResampling, ptSpr.sprAngle, ptSpr.sprScaleX, ptSpr.sprScaleY, bHotSpot, ptSpr.sprX, ptSpr.sprY, ptSpr.sprEffect, ptSpr.sprEffectParam);
						}
					}
				}
			}
